...
export default {
    ...
    login: async (_, { input }) => {
      ...
      try {
        ...
        const login = await db1.oneOrNone(
          `SELECT co_usuario, usuario, nb_usuario, nb2_usuario, ap_usuario, ap2_usuario, cedula_usr, gerencia, tx_correo, co_rol, created_at
  ...
  Mutation: {
    inserNewUser: async (_, { input }, { auth }) => {
      if (!auth) throw new ApolloError('Sesión no válida')
